...
MyGradient::MyGradient(GPoint p0, GPoint p1, const GColor c[], int count) 
    :   lm(setLM(p0, p1)),
        colorCount(count), 
...
void MyGradient::shadeRow(int x, int y, int count, GPixel row[]) {
    ...
    if(colorCount == 2) {
        ...
        GColor newC; float t;
        for(int i = 0; i < count; ++i) {
            t = std::min(std::max(currX, 0.f), 1.f);
    ...
    for(int i = 0; i < count; ++i) {
        newX = std::min(std::max(currX, 0.f), 1.f) * (colorCount - 1);
        k = GFloorToInt(newX);
        t = newX - k;
...
std::unique_ptr<GShader> GCreateLinearGradient(GPoint p0, GPoint p1, const GColor colors[], int count) {
    return std::unique_ptr<GShader>(new MyGradient(p0, p1, colors, count));
};