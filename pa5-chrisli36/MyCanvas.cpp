...
void MyCanvas::drawPath(const GPath& path, const GPaint& paint) {
        ...
        switch(v.value()) {
            ...
            case GPath::kQuad:
                ...
                for(int seg = 0; seg < numSegs; ++seg) {
                    clipEdge(computeQuad(seg / numSegs, edge[0], edge[1], edge[2]), computeQuad((seg + 1) / numSegs, edge[0], edge[1], edge[2]), edgeStorage);
                }
                break;
            case GPath::kCubic:
                ...
                for(int seg = 0; seg < numSegs; ++seg) {
                    clipEdge(computeCubic(seg / numSegs, edge[0], edge[1], edge[2], edge[3]), computeCubic((seg + 1) / numSegs, edge[0], edge[1], edge[2], edge[3]), edgeStorage);
                }
                break;
        }
    }
    ...
    GIRect boundingRect = newPath.bounds().roundOut();
...
std::string GDrawSomething(GCanvas* canvas, GISize dim) {
    ...
    for(int i = 0; i < 1000; ++i) {
        ...
        shader = new MyGradient((GPoint) {0, 0}, (GPoint) {15, 15}, c, 2, GTileMode::kMirror);
        paint->setShader(shader);
        canvas->drawRect(GRect::XYWH(x, y, w, h), *paint);
    }
    return "shiny gradients";
}
