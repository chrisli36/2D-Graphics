...
static void kSrcDst(int left, int right, int y, GPixel& src, const GBitmap& fDevice) { // S + D
    GPixel* dst;
    for(int x = left; x < right; ++x) {
        dst = fDevice.getAddr(x, y);
        *dst = GPixel_PackARGB(GPixel_GetA(src) + GPixel_GetA(*dst), 
...
static void kDstASrc(int left, int right, int y, GPixel& src, const GBitmap& fDevice) { // Da*S + D
    GPixel* dst;
    int dstA;
    for(int x = left; x < right; ++x) {
        dst = fDevice.getAddr(x, y);
        dstA = GPixel_GetA(*dst);
        if(dstA == 0) {
            *dst = GPixel_PackARGB(GDiv255(GPixel_GetA(*dst)), 
                                    GDiv255(GPixel_GetR(*dst)), 
                                    GDiv255(GPixel_GetG(*dst)), 
                                    GDiv255(GPixel_GetB(*dst)));
        } else {
            *dst = GPixel_PackARGB(GDiv255(dstA * GPixel_GetA(src)) + GPixel_GetA(*dst), 
                                    GDiv255(dstA * GPixel_GetR(src)) + GPixel_GetR(*dst), 
                                    GDiv255(dstA * GPixel_GetG(src)) + GPixel_GetG(*dst), 
                                    GDiv255(dstA * GPixel_GetB(src)) + GPixel_GetB(*dst));
        }
    }
}
